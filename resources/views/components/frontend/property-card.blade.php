<div class="card property-card h-100 border-0 shadow-sm overflow-hidden">
    ...
    {{-- Property Image --}}
    <a href="{{ route('properties.city.slug', [$property->city->slug, $property->slug]) }}" class="text-decoration-none">

        @if ($property->featured_image)
            <img src="{{ asset('storage/properties/featured/' . $property->featured_image) }}"
                alt="{{ $property->title }}" class="card-img-top w-100" style="height: 240px; object-fit: cover;">
        @else
    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            @if ($property->availability)
                <span
                    class="badge
                @if ($property->availability === 'available') bg-success-subtle text-success
                @elseif($property->availability === 'sold')
                    bg-danger-subtle text-danger
                @elseif($property->availability === 'rented')
                    bg-warning-subtle text-warning-emphasis
                @else
                    bg-info-subtle text-info @endif
            ">
